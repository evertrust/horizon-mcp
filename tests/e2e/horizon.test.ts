...
describe.skipIf(!E2E_CONFIGURED)('Horizon E2E', () => {
  ...
  describe('discovery', () => {
    ...
    describe('discovery import workflow', () => {
      // Name of a pre-existing discovery campaign on the target instance.
      // The test is skipped when the variable is unset.
      const QA_CAMPAIGN = process.env['HORIZON_E2E_DISCOVERY_CAMPAIGN'] ?? '';

      it.skipIf(!QA_CAMPAIGN)(
        'feeds a cert into a QA campaign and verifies import',
        async () => {
