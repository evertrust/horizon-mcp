 ...
 *   - delete blocked (403 AUTOMATION-POLICY-005) only when referenced by a valid cert
 *
 ...
 * is one of est/scep/webra/acme/acme-external. A full setup would build a
 * brand-new EST profile, which itself needs an integrated PKI connector
 ...
 * surfacing an AUTOMATION-POLICY-* / profile-reference / "not found" server code,
 * NOT a tool/client bug), and the round-trip + update assertions short-circuit.
 *
 * A full payload also seeds compliancePolicy.authorizedCas with a CA that has
 ...
 * empty {} block via update (the API accepts {}).
 */
...
// Profile modules that an automation policy accepts.
  ...
  () => {
    ...
    it('creates the automation policy (or reports a clean validation error)', async () => {
      if (!profileName) {
        ...
        try {
          ...
          expect((err as ToolError).message).toMatch(
            /AUTOMATION-POLICY-|profile|reference|not found|does not exist/i,
          );
        }
        return;
      }
      ...
      try {
        ...
        expect(err).toBeInstanceOf(ToolError);
        expect((err as ToolError).message).toMatch(
          /AUTOMATION-POLICY-|profile|trusted|license|not licensed|forbidden|unauthorized/i,
        );
      }
    });
    ...
    it('updates it (adds an empty compliance_policy block)', async () => {
      if (!created) return;
      // The API accepts an empty compliancePolicy {}. This avoids
    ...
    it('confirms deletion: get now errors (AUTOMATION-POLICY-003 not found)', async () => {
