 ...
 *
 * The valid create body comes from the tool schema + the unit-test golden
 * mapping (tests/unit/config-execution-policies.test.ts), which encode the exact
 ...
 *   dateRange / timeRange are {start, end} objects (the object form the API
 *   accepts, not the OpenAPI single-string shape).
 *
 ...
 * untouched fields persist and the response-only `_id` is stripped before PUT.
 *
...
describe.skipIf(!E2E_CONFIGURED)('execution_policy CRUD E2E (live QA)', () => {
  ...
  it('is gone after delete', async () => {
    ...
    // (EXECUTION-POLICY-003), raised as a ToolError - not a tool/client bug.
    let err: unknown;
    try {
