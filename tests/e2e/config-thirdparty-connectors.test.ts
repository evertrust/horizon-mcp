 ...
 * SIMPLEST subtype: `aws`.
 *
 * Why aws:
 *   - Its only mandatory subtype field beyond the universal trio is `region`.
 *   - `credentials` is OPTIONAL for aws (env fallback) per the tool schema,
 ...
 *     fields for aws; they are ABSENT from the resolved AWSConnector schema, so the client-side guard (assertConfigBody) rejects
 *     them as unknown keys. The tool schema is authoritative here, so the
 *     minimal valid body is type/name/throttleDuration/region.
 *
 * Minimal valid payloads:
 *     - create -> { type: aws, name, throttleDuration: "60s", region: "FR" }
 *     - delete -> DELETE /{name} (204)
 *     - password credential
...
describe.skipIf(!E2E_CONFIGURED)(
  ...
  () => {
    ...
    // Minimal valid aws connector body (per the tool schema).
    ...
    beforeAll(async () => {
      ...
      // directly via the raw HorizonClient.
      try {
    ...
    it('creates an aws connector (or reports a clean server validation/license error)', async () => {
      ...
      try {
        ...
        expect((err as ToolError).message).toMatch(
          /THIRDPARTY-CONNECTOR-|thirdparty|connector|licen[cs]e|region|credential|reference|not\s+(allowed|enabled|supported)/i,
        );
      }
    });
    ...
    it('deletes it (with expected_name safeguard)', async () => {
      ...
      // Confirms deletion: get now errors (THIRDPARTY-CONNECTOR-003 not found).
      await expect(
