...
describe.skipIf(!E2E_CONFIGURED)('triggers CRUD E2E (live QA)', () => {
  ...
  // Minimal email subtype body.
  const createConfig = {
    events: ['on_enroll'],
    emailTemplate: {
      to: [{ type: 'static', email: 'test@example.com' }],
  ...
  // Update body - flips isHtml to true and adds an HTML body. Full config
  // re-supplied (PUT full-replace).
  const updateConfig = {
    events: ['on_enroll'],
    emailTemplate: {
      to: [{ type: 'static', email: 'test@example.com' }],
