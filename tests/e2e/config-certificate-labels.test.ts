 ...
 * mapped to the MCP tool's snake_case inputs. A `regex` field is NOT part of the
 * OpenAPI Label request schema (#/components/schemas/Label only exposes
 * name/displayName/description), so the MCP create tool does not surface it
 * and the server ignores it; we omit it.
 *
...
describe.skipIf(!E2E_CONFIGURED)('certificate_label CRUD E2E (live QA)', () => {
  ...
  it('creates a certificate label', async () => {
    // Minimal body: { name, displayName: [], description: [] }.
  ...
  it('updates the localized display name', async () => {
    // Beyond a plain round-trip (which resends empties), set a real localized
  ...
  it('is gone after delete', async () => {
    // GET on a deleted label must surface a clean Horizon 404 (CERT-LABEL-003),
