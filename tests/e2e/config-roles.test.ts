 ...
 *
 * IMPORTANT - dependency avoidance: `lifecycle:*` permissions reference a
 * profile or PKI connector that must already exist. Horizon rejects roles
 * referencing profiles / discovery campaigns / SCIM profiles that do not exist
 * (ROLE-002).
 ...
 *
 * Membership: add a throwaway identifier (non-existing
 ...
 *
 * Role name regex is [0-9a-zA-Z-_]+ (no dots, no spaces); the E2E_PREFIX form
 * satisfies it.
 *
...
describe.skipIf(!E2E_CONFIGURED)('role CRUD E2E (live QA)', () => {
  ...
  // Initial permission set (dependency-free configuration:* values only).
  ...
  // Updated, smaller permission set (reduces the list down to CA/PKI
  // permissions).
