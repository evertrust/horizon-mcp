 ...
 *     - update                -> PUT collection, host change
 *     - password credential   -> proxy Basic-auth credentials dep
 *     - update with creds     -> PUT collection adding credentials
 *
 * Tool schema:
...
describe.skipIf(!E2E_CONFIGURED)('http_proxies CRUD E2E (live QA)', () => {
  ...
  // Credentials name uses the same name rules; prefix keeps it unique.
  ...
  // Minimal create and update values.
  ...
  beforeAll(async () => {
    ...
    // HorizonClient. Minimal valid payload:
  ...
  it('creates and round-trips a credentials-backed proxy (or reports a clean validation error)', async () => {
    ...
    // HTTP-PROXY-002 - which is a legitimate server validation error, NOT a
    // tool/client bug. We tolerate that case.
    const createArgs = {
      name: authProxyName,
    ...
    if (!credsCreated) {
      ...
      try {
        ...
        expect((err as ToolError).message).toMatch(
          /HTTP-PROXY-002|credential|reference/i,
        );
      }
      return;
    }
    ...
    expect(fetched['host']).toBe('proxy.example.com');
