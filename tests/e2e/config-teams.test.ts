 ...
 *           -> 201, body.name defined.
 *   update: adds contact "contact@example.com"
 ...
 *
 * switch_team semantics:
 *   PATCH /api/v1/security/teams/{previousTeam}/{newTeam} requires newTeam to
 *   ALREADY EXIST (404 TEAM-003 otherwise). It reassigns certificates + requests
 ...
 *
 * Team name regex is [0-9a-zA-Z-_]+ (no dots, no spaces); the E2E_PREFIX form
 * (e2e-<hex8>-team) satisfies it.
 *
...
describe.skipIf(!E2E_CONFIGURED)('team CRUD E2E (live QA)', () => {
  ...
  // Webhook structure matches what Horizon validates.
  const webhook = {
    type: 'slack' as const,
    url: 'https://example.com/redacted-webhook',
  };

  // contact added by the update step; server-validated e-mail.
  ...
  it('is gone after delete', async () => {
    // GET on a deleted team must surface a clean Horizon 404 (TEAM-003),
