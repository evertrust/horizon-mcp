 ...
 *
 * Grading has NO create/update/delete request body over the REST API: only
 * GET list / GET by name exist (plus explain/run, which do not persist).
 * Grading policies and rulesets are system-bootstrapped defaults (the built-in
 * 'Horizon-Grading-Policy' is expected to exist).
 *
 ...
 *
 * Tolerance: a standard QA tenant has the grading defaults, so the built-in
 * policy round-trip is expected to succeed. If the instance lists no policies,
 * the get round-trip is skipped rather than failing - the list/get tools
 * themselves are still asserted to behave correctly.
 */
import { describe, expect, it } from 'vitest';

import { E2E_CONFIGURED, callTool, setupE2EStack } from './setup.js';

// The built-in grading policy.
...
describe.skipIf(!E2E_CONFIGURED)('grading read-only E2E (live QA)', () => {
  ...
  it('gets a single grading policy by name (built-in or first listed)', async () => {
    ...
    // The instance may list zero policies; nothing to round-trip in that case.
