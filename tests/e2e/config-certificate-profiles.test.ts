 ...
 *
 * The create body is a monitored profile with the four mandatory nested policy
 * objects (authorizationLevels / requestsPolicy / selfPermissions /
 * cryptoPolicy) plus a few optional structural fields. Two keys are
 * intentionally left out:
 *   - `notifications`: NOT a monitored profile field and NOT in the MCP tool's
 *     KNOWN_KEYS allow-list -> assertConfigBody would reject it. The server
 *     ignores it anyway.
 *   - `pkiConnector`: the server STRIPS it on a monitored profile. Omitting it
 *     avoids referencing a connector that may not exist on this QA instance;
 *     monitored create succeeds without it.
 * `validationRuleset` is likewise stripped on the response but is an
 * allow-listed key, so we pass it.
 *
 ...
 * collection root; module/name immutable). Create and update share the same
 * body; update PUTs the collection.
 *
 ...
 * ToolError surfacing a real CERT-PROFILE-* / license server code and skip
...
describe.skipIf(!E2E_CONFIGURED)(
  ...
  () => {
    ...
    // The four mandatory nested policy objects. authorizationLevels carries the
    ...
    // Remaining top-level fields, all in the tool's KNOWN_KEYS
    ...
    it('creates a monitored certificate profile', async () => {
      try {
        ...
        // (a real CERT-PROFILE-* server code), but never a tool/client bug.
        expect(err).toBeInstanceOf(ToolError);
        expect((err as ToolError).message).toMatch(
          /CERT-PROFILE-|monitored|license|not licensed|entitlement|forbidden|unauthorized/i,
        );
      }
    });
    ...
    it('is gone after delete', async () => {
      ...
      // (CERT-PROFILE-003), raised as a ToolError - not a tool/client bug.
      let err: unknown;
      try {
