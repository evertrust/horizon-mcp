...
describe("HCQL extraction", () => {
  ...
  it("extracts next 7 days", () => {
    expect(
      fragments(
        extractHcql("certificates expiring in the next 7 days"),
      ),
  ...
  it("extracts last 24 hours", () => {
    expect(
      fragments(
        extractHcql("certificates issued in the last 24 hours"),
      ),
  ...
  it("extracts owner field", () => {
    expect(
      fragments(
        extractHcql("certificates where owner is admin@corp.io"),
      ),
  ...
  it("converts weeks to days", () => {
    expect(
      fragments(
        extractHcql("certificates expiring in the next 2 weeks"),
      ),
  ...
  it("converts months to days", () => {
    expect(
      fragments(
        extractHcql("certificates issued in the last 3 months"),
      ),
...
describe("HRQL extraction", () => {
  ...
  it("extracts revocation workflow with date", () => {
    const f = fragments(
      extractHrql("revocation requests from last 7 days"),
    );
  ...
  it("extracts profile field", () => {
    expect(
      fragments(extractHrql("requests for profile ACME-Prod")),
    ).toContain('profile equals "ACME-Prod"');
  });
...
describe("HEQL extraction", () => {
  ...
  it("extracts last 24h timestamp", () => {
    expect(
      fragments(extractHeql("events in the last 24 hours")),
    ).toContain("timestamp after -24h");
  });
  ...
  it("extracts ACME module filter", () => {
    expect(
      fragments(extractHeql("ACME events from last 24 hours")),
    ).toContain('module equals "ACME"');
  });
  ...
  it("extracts ACME revocation module filter", () => {
    expect(
      fragments(extractHeql("ACME revocation events")),
    ).toContain('module equals "ACME"');
  });
  ...
  it("extracts request approval event code", () => {
    expect(
      fragments(extractHeql("request approval events")),
    ).toContain('code equals "REQUEST-APPROVE"');
  });

  it("extracts actor detail", () => {
    expect(
      fragments(
        extractHeql("events by admin@corp.io in the last 7 days"),
      ),
...
describe("HDQL extraction", () => {
  ...
  it("extracts hostname", () => {
    expect(
      fragments(extractHdql("discovery on host example.com")),
    ).toContain('hostname equals "example.com"');
  });

  it("converts hostname glob to regex", () => {
    expect(
      fragments(extractHdql("discovery on host *.example.com")),
    ).toContain('hostname matches ".*\\.example\\.com"');
  });

  it("extracts campaign", () => {
    expect(
      fragments(extractHdql("discovery campaign weekly-scan")),
    ).toContain('campaign equals "weekly-scan"');
  });
});
...
describe("Tool invocation via MCP", () => {
  ...
  beforeAll(async () => {
    const server = new McpServer(
      { name: "translate-test", version: "0.0.0" },
    );
