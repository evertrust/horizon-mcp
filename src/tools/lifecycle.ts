...
export function registerLifecycleTools(
  ...
  server.registerTool(
    ...
    async ({ certificate_id, format }) => {
      ...
      const pem =
        cert["certificate"] ?? cert["pem"] ?? cert["certificatePEM"];
      if (!pem) {
        return {
          content: [
            {
    ...
    "get_request_template",
    {
      ...
      inputSchema: z.object({
        ...
        module: z
          ...
          .optional()
          .describe(
            "Profile module (webra, est, scep, acme, etc.).",
          ),
        profile: z
          .string()
          .optional()
  ...
  server.registerTool(
    ...
    async ({ workflow, profile, module, template, password, certificate_id, data }) => {
