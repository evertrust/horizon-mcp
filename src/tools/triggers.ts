...
function validateSequenceStep(
  ...
  if (
    ...
    !VALID_METHODS.has(method.toUpperCase())
  ) {
    return JSON.stringify({
  ...
  const auth =
    (step["authenticationType"] as string | undefined) ?? "noauth";
...
export function registerTriggerTools(
  ...
  server.registerTool(
    ...
    async ({ max_items, name_contains, credential_type }) => {
      ...
      if (credential_type !== undefined && !validCredTypes.has(credential_type)) {
        return {
          content: [
            {
              type: "text" as const,
    ...
    "create_rest_notification",
    {
      ...
      inputSchema: z.object({
        ...
        retries: z
          ...
          .int()
          .default(10)
          .describe("Retry count on failure with exponential backoff (default 10)."),
        run_period: z
          .string()
          .optional()
          .describe(
        ...
        on_trigger_error: z
          ...
          .optional()
          .describe(
            "Names of triggers to fire if this notification fails.",
          ),
      }),
    },
    async ({
  ...
  server.registerTool(
    "simulate_trigger",
    {
      ...
      inputSchema: z.object({
        name: z
          .string()
          .describe("Name of the existing trigger to simulate."),
      }),
    },
    async ({ name }) => {
