 ...
 *     per-object strip set, never the stale global STRIP_FIELDS map)
 ...
 *   - COMPLEX / polymorphic: a `describe_<noun>_schema` tool surfaces the resolved
 ...
 *
 * All contracts here are derived from the Horizon API reference.
 */
...
export interface ConfigSpec {
  ...
  /** Server-populated fields to strip before a PUT (the per-object strip set). */
