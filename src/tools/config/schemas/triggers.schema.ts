...
export const triggerRequestSchema = {
  ...
  $defs: {
    ...
    EmailRecipient: {
      ...
      properties: {
        type: {
          type: 'string',
          description:
            "The type of email recipient. Apart from 'static', all are deduced from the request context. One of 11 values; the OpenAPI EmailRecipient enum omits 'team_members'.",
          enum: [
            'static',
            'team_contact',
            'label',
    ...
    MapEntry: {
      type: 'object',
      description: 'Used for email headers.',
      properties: {
        key: { type: 'string' },
        value: { type: 'string' },
      },
    ...
    EmailTemplate: {
      ...
      description:
        "'cc', 'bcc', 'headers' are accepted but not documented in the OpenAPI EmailTemplate request schema.",
      properties: {
        to: {
          type: 'array',
          nullable: true,
          items: { $ref: '#/$defs/EmailRecipient' },
          description: "Recipients. Server validation requires non-empty 'to'.",
        },
        cc: {
          type: 'array',
          nullable: true,
        ...
        from: {
          type: 'string',
          description: 'Sender address. Must be a valid email.',
        },
        title: {
          type: 'string',
    ...
    EmailNotification: {
      ...
      properties: {
        ...
        events: {
          ...
          description:
            'MUST contain exactly one value. Each value must be in the subtype authorizedEvent set.',
        },
        retries: {
          type: 'integer',
          nullable: true,
          description: '0 up to the server-configured maximum retries.',
        },
        runPeriod: {
