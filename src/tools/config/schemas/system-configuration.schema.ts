 ...
 *
 * Build-time constant resolved from the bundled OpenAPI. The body is a discriminated
...
export const systemConfigurationRequestSchema = {
  ...
  description:
    'Self-contained resolved JSON Schema for the PUT (upsert) request body of the Horizon System Configuration singleton-per-type object. The body is a discriminated union keyed on the `type` field. Resolved from bundled OpenAPI components (LicenseConfiguration, InternalMonitorConfiguration, InterfaceCustomizationConfiguration, StorageConfiguration). `_id` and `tenant` are server-managed and not part of the request body.',
  oneOf: [
  ...
  $defs: {
    LicenseConfiguration: {
      ...
      properties: {
        ...
        triggers: {
          description:
            'Triggers to execute on license events. Optional (defaults to null/absent). Server validates that every referenced trigger name exists AND is runnable on the corresponding event; otherwise the server returns HTTP 400.',
          type: ['object', 'null'],
    ...
    InternalMonitorConfiguration: {
      ...
      properties: {
        ...
        cron: {
          type: 'string',
          description:
            'Quartz Cron expression defining when to run internal monitor checks. MANDATORY (no default). Must be a valid Quartz cron expression; an invalid expression is rejected with HTTP 400.',
          examples: ['0 0 0 ? * * *', '0 /1 * ? * *'],
        },
      },
      required: ['type', 'cron'],
    ...
    Announcement: {
      ...
      properties: {
        ...
        content: {
          type: 'array',
          description:
            'The announcement contents. Server-enforced: MUST contain at least one element.',
          minItems: 1,
    ...
    StorageConfiguration: {
      ...
      properties: {
        type: {
          ...
          const: 'storage',
          description:
            'The discriminator. Selects this subtype. NOTE: the body discriminator value is the literal `storage`. The GET /{type} path parameter enum lists `storage_configuration`, which is a documented mismatch in the bundled OpenAPI; the actual stored/discriminated type value is `storage`.',
        },
        archiveStorage: {
          type: ['string', 'null'],
          description:
            "Name of a system storage to use for archive file storage. Optional. If set, MUST reference an existing storage (system.storage object); otherwise the server returns HTTP 400. The referenced storage's type must be allowed by the server configuration.",
        },
