 ...
 *
 * Build-time constant resolved from the bundled OpenAPI. Surfaced
...
export const scheduledTaskRequestSchema = {
  ...
  $defs: {
    ...
    HQLType: {
      ...
      description:
        'Query language for the report. Note: hpql is accepted by the enum but only hcql/hrql/heql/hdql are supported; report validation rejects hpql.',
    },
    ReportRecipientType: {
      type: 'string',
    ...
    FiniteDuration: {
      ...
      description: "Finite duration string, e.g. '10s', '7 days'.",
    },
    ReportRecipient: {
      type: 'object',
    ...
    ThirdPartyScheduledTask: {
      ...
      properties: {
        ...
        renew: {
          type: 'boolean',
          description:
            'Required (no default), although the OpenAPI omits it from the required list.',
        },
        description: { type: ['string', 'null'] },
        _id: {
          type: 'string',
          description:
            'Server-populated internal id. Ignored on write. Present only on responses.',
        },
        host: {
          type: ['string', 'null'],
    ...
    ReportCommon: {
      ...
      properties: {
        ...
        from: {
          type: 'string',
          description:
            'Sender email. Validated server-side as a real email address.',
        },
        ...
        cc: {
          type: ['array', 'null'],
          items: { $ref: '#/$defs/ReportRecipient' },
          description: 'Accepted but not in the OpenAPI. Optional.',
        },
        bcc: {
          type: ['array', 'null'],
          items: { $ref: '#/$defs/ReportRecipient' },
        ...
        fileName: {
          ...
          pattern: '^[0-9a-zA-Z\\-_\\.]+$',
          description:
            'Optional; when set must match [0-9a-zA-Z-_.]+ else rejected (HTTP 400).',
        },
        ...
        headers: {
          ...
          description:
            'Accepted but not in the OpenAPI. Optional email headers.',
        },
        description: { type: ['string', 'null'] },
        _id: {
          type: 'string',
