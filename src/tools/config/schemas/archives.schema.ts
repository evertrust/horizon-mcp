 ...
 *
 * Build-time constant resolved from the bundled OpenAPI.
    ...
    CertificateArchive: {
      ...
      properties: {
        ...
        filter: {
          type: ['string', 'null'],
          description:
            "Optional HCQL filter selecting which certificates to archive. Validated server-side as HCQL; an invalid filter yields ARCHIVE-002 'Invalid archive'.",
        },
      },
    ...
    EventArchive: {
      ...
      properties: {
        ...
        before: {
          ...
          description:
            "Epoch milliseconds. Date before which all events will be archived. Server validation: must be earlier than now minus the configured event grace period, and event archiving is rejected entirely when horizon.event.ttl is configured. Violations yield ARCHIVE-002 'Invalid archive'.",
        },
      },
