 ...
 * the effective primary key (unique) AND immutable. The single
 * PUT /api/v1/system/configuration is an UPSERT (full replace of the entry of
 * that type). The wrapper does GET-strip-merge so omitted fields are preserved on
 * update. stripFields = [_id, tenant].
 *
