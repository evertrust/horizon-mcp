 ...
 * server-derived fields (_id, tenant).
 *
 * Object-specific rule: clusterWide is an optional create param defaulting to
 * false (matching the server default) and is ALWAYS sent explicitly on create,
 * so it is not listed as a mandatory field the model must ask the user about.
 */
...
const throttleDurationSchema = z
  .string()
  .describe(
    'Optional throttle window as a finite duration string, e.g. "5 seconds". ' +
