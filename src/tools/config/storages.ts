/**
 * Storage backend configuration tools (S3; fully typed - only the s3 subtype
 * exists today).
 *
 * 5 tools: list / get / create / update / delete.
 * Body fields per the Horizon OpenAPI. NOTE: the API requires forcePathStyle,
 * bucket, checksumMode and partBufferSize (no default), even though the
 * OpenAPI omits checksumMode from its required array - so they are mandatory
 * here. timeout is server-enforced mandatory.
 *
...
export function registerStorageTools(
  ...
  registerDeleteTool(server, client, SPEC, {
    ...
    deleteConstraints:
      ...
      'archiveStorage or magicLinkReportStorage.',
  });
}
