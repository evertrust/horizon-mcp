 ...
 * Body fields per the Horizon OpenAPI. POST and PUT share the exact same body
 * schema.
 *
 * Route: /api/v1/automation/policies. Update PUTs the COLLECTION root (body-keyed
 * full-replace on name); the wrapper does GET-merge so omitted
...
export function registerAutomationPolicyTools(
  ...
  registerDeleteTool(server, client, SPEC, {
    ...
    deleteConstraints:
      ...
      '(certificate metadata.automationPolicy == name) (AUTOMATION-POLICY-005).',
  });
}
