 ...
 * name and module are immutable after creation.
 */
...
/**
 * Top-level mandatory keys common to (nearly) every subtype, per the resolved
 * request schema. assertConfigBody enforces these are present on the
...
export function registerCertificateProfileTools(
  ...
  registerDeleteTool(server, client, SPEC, {
    ...
    deleteConstraints:
      ...
      'CERT-PROFILE-006 (403) on an invalid reference.',
  });
}
