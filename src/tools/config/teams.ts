 ...
 *
 * Body fields per the Horizon OpenAPI. The request schema is tractable, so every
 * field is a typed Zod param (snake_case input -> exact camelCase API key).
 *
...
export function registerTeamTools(
  ...
  registerDeleteTool(server, client, SPEC, {
    ...
    deleteConstraints:
      'Blocked (400) if the team is referenced by a SCIM profile ' +
