...
const DEFAULT_DATE_FIELDS: Readonly<Record<QueryType, { future: string; past: string }>> = {
...
function extractDateConditions(
  ...
  m = text.match(
    /expir\w*\s+(?:in\s+)?(\d+)\s+(day|hour|minute|week|month)s?/,
  );
  if (m) {
    const dur = toDuration(parseInt(m[1]!, 10), m[2]!);
    conditions.push(condition(
...
const FIELD_PATTERNS: Readonly<Record<string, readonly FieldPattern[]>> = {
  hcql: [
    ["profile\\s+(?:named?\\s+|called\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "profile", "profile"],
    ["team\\s+(?:named?\\s+|called\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "team", "team"],
    ["owner\\s+(?:is\\s+|=\\s*)?[\"']?([\\w@._-]+)[\"']?", "owner", "owner"],
    ["issuer\\s+(?:is\\s+|=\\s*|contains?\\s+)?[\"']?([\\w\\s.*=,-]+?)[\"']?(?:\\s+and\\b|\\s*$)", "issuer", "issuer"],
    ["(?:subject|dn)\\s+(?:match(?:es|ing)?\\s+|contains?\\s+|=\\s*)?[\"']?([\\w\\s.*=,-]+?)[\"']?(?:\\s+and\\b|\\s*$)", "dn", "subject/DN"],
    ["(?:module|connector)\\s+(?:named?\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "module", "module"],
    ["(?:san|subject\\.alt)\\w*\\s+(?:contains?\\s+|match(?:es|ing)?\\s+)?[\"']?([\\w.*@-]+)[\"']?", "san", "SAN"],
  ],
  hrql: [
    ["profile\\s+(?:named?\\s+|called\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "profile", "profile"],
    ["team\\s+(?:named?\\s+|called\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "team", "team"],
    ["requester\\s+(?:is\\s+|=\\s*)?[\"']?([\\w@._-]+)[\"']?", "requester", "requester"],
    ["owner\\s+(?:is\\s+|=\\s*)?[\"']?([\\w@._-]+)[\"']?", "owner", "owner"],
    ["(?:module|connector)\\s+(?:named?\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "module", "module"],
  ],
  heql: [
    ["(?:module|connector)\\s+(?:named?\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "module", "module"],
    ["node\\s+(?:named?\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "node", "node"],
  ],
  hdql: [
    ["source\\s+(?:is\\s+|=\\s*)?[\"']?([\\w][\\w.-]*)[\"']?", "source", "source"],
  ],
};
...
function extractFieldValues(
  ...
  for (const [pattern, hqlField, label] of patterns) {
    ...
    if (m) {
      ...
      conditions.push(condition(
        `${hqlField} ${op} "${val}"`,
        op === "matches" ? `${label} matching '${raw}'` : `${label} is '${raw}'`,
      ));
    }
  }
}
...
export function extractHcql(text: string): Condition[] {
  ...
  for (const [status, pat] of statusMap) {
    if (new RegExp(`\\bnot?\\s+${pat}\\b`).test(lower)) {
      conditions.push(condition(`status is not ${status}`, `non-${status} certificates`));
    } else if (new RegExp(`\\b${pat}\\b`).test(lower)) {
      // Avoid matching "valid.until" as a status reference
      if (status === "valid" && /valid\.\w+/.test(lower)) {
        continue;
      }
      conditions.push(condition(`status is ${status}`, `${status} certificates`));
    }
  }
  ...
  for (const [prop, pat] of props) {
    if (new RegExp(`\\bnot?\\s+${pat}\\b`).test(lower)) {
      ...
      conditions.push(condition(`certificate is ${prop}`, `${prop} certificates`));
    }
  }
  ...
  for (const [kt, pat] of keyTypes) {
    if (new RegExp(`\\b(?:${pat})\\b`).test(lower)) {
      conditions.push(condition(`keytype contains "${kt}"`, `${kt.toUpperCase()} key type`));
      break;
    }
  }
  ...
  if (/trigger.*fail|failed?\s+trigger/.test(lower)) {
    conditions.push(condition("trigger.results has failure", "failed triggers"));
  } else if (/trigger.*warn/.test(lower)) {
    conditions.push(condition("trigger.results has warning", "trigger warnings"));
  }
...
export function extractHrql(text: string): Condition[] {
  ...
  for (const [status, pat] of statusMap) {
    if (new RegExp(`\\b${pat}\\b`).test(lower)) {
      conditions.push(condition(`status equals "${status}"`, `${status} requests`));
      break;
    }
  }
...
export function extractHeql(text: string): Condition[] {
  ...
  if (!moduleFound) {
    const codeMap: [string, string, string][] = [
      ...
      // --- Request events ---
      ["\\brequest.*submit|submit.*request", "REQUEST-SUBMIT", "request submission"],
      ["\\brequest.*approv|approv.*request", "REQUEST-APPROVE", "request approval"],
      ["\\brequest.*deny|deny.*request|denied", "REQUEST-DENY", "request denial"],
      ["\\brequest.*cancel|cancel.*request", "REQUEST-CANCEL", "request cancellation"],
      // --- Security events ---
      ...
      ["\\bconfig.*delet|config.*remov", "CONF-DELETE", "configuration deletion"],
  ...
  const certPattern = /(?:certificate|cert)\s+(?:named?\s+|called\s+|for\s+)?["']?([\w][\w.*-]*(?:\.[\w.*-]+)*)["']?/;
...
export function extractHdql(text: string): Condition[] {
  ...
  if (m) {
    ...
    if (hostname.includes("*")) {
      ...
      conditions.push(condition(`hostname matches "${regex}"`, `hostname matching ${hostname}`));
    } else {
      conditions.push(condition(`hostname equals "${hostname}"`, `hostname ${hostname}`));
    }
  }
  ...
  if (m) {
    conditions.push(condition(`campaign equals "${m[1]}"`, `campaign '${m[1]}'`));
  }
  ...
  server.registerTool(
    "translate_to_hql",
    {
      ...
      inputSchema: z.object({
        natural_language: z.string().describe(
        ...
        target_type: z.string().optional().describe(
    ...
    async ({ natural_language, target_type, validate }) => {
      ...
      const avgConf = conditions.reduce((sum, c) => sum + c.confidence, 0) / conditions.length;
      const overall = Math.round(Math.min(intentConfidence, avgConf) * 100) / 100;
