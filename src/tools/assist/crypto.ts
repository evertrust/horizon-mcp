...
const FETCH_EXPOSED_CERTIFICATE_CONFIG = {
  description:
...
const CONVERT_PKCS12_TO_JKS_CONFIG = {
  description:
...
export function registerCryptoTools(
  ...
  registerTool(server, 'decode_x509', DECODE_X509_CONFIG, async ({ pem }) => {
  ...
  registerTool(server, 'decode_csr', DECODE_CSR_CONFIG, async ({ pem }) => {
  ...
  registerTool(server, 'decode_crl', DECODE_CRL_CONFIG, async ({ data }) => {
  ...
  registerTool(server, 'decode_ocsp', DECODE_OCSP_CONFIG, async ({ data }) => {
  ...
  registerTool(server, 'decode_tsa', DECODE_TSA_CONFIG, async ({ data }) => {
  ...
  registerTool(server, 'detect_file', DETECT_FILE_CONFIG, async ({ data }) => {
  ...
  registerTool(
    server,
    'fetch_exposed_certificate',
    FETCH_EXPOSED_CERTIFICATE_CONFIG,
    async ({ uri, timeout }) => {
