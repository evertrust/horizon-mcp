...
| **Monitored** | Monitored (CertMonitor)                                                       | Horizon *observes* externally-issued certificates but does not issue them.  |
...
| `authorizationLevels`            | object    | WHO can perform each workflow action (enroll, revoke, update, etc.). See *workflows* knowledge.  |
...
The `certificateTemplate` object defines the *defaults* for certificates
...
Monitored profiles do not issue certificates. They define a *bucket* for
...
```json
{
  "cryptoPolicy": {
    "minimumKeySize": 2048,
    "allowedSigningAlgorithms": ["SHA256WithRSA", "SHA384WithRSA", "SHA256WithECDSA"]
  }
}
```
...
```json
{
  ...
  "certificateTemplate": {
    "subject": [
      ...
      {"type": "O", "value": "Acme Corp", "mandatory": true, "editableByRequester": false},
      {"type": "C", "value": "US", "mandatory": true, "editableByRequester": false}
    ],
    "sans": [
    ...
    "contactEmailPolicy": {"editable": true, "required": true, "autoFill": true},
    "metadataPolicies": [
      {"key": "environment", "editable": true, "required": true, "regexValidation": "^(prod|staging|dev)$"}
    ],
    "labels": [
      {"label": "use-case", "value": "tls", "editable": false}
    ]
  },
  "authorizationLevels": {
    "search": {"accessLevel": "authenticated"},
