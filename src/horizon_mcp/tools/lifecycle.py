...
def register_lifecycle_tools(mcp: FastMCP) -> None:
    ...
    async def download_certificate(
        ...
        Only PEM format is available through the API. The certificate PEM
        is extracted from the certificate details response.
        For DER, PKCS12, or JKS formats, use the Horizon UI.
        """
        client = get_client()
        fmt = format.lower()
        if fmt != "pem":
    ...
    async def submit_request(
        ...
        Args:
            ...
            password: Password for private key protection (centralized key generation).
