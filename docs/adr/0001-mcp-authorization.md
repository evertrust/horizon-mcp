...
Contract 6 blocks all the others. The observed behavior of the Horizon 2.10
API shows that **no bridge exists today**. A bridge requires a Horizon-side
change, not a horizon-mcp change.
...
1. **No token exchange.** RFC 8693 is an authorization-server feature and
   ...
   call.
2. **No bearer authentication on the API.** Horizon accepts an API key
   (`X-API-ID`/`X-API-KEY`), a service-account token (`X-API-SVA` plus
   `X-API-TOKEN`), a client certificate, or a browser session cookie. An OIDC
   access token sent as a Bearer header is not among them.
3. **The federated JWT path does not carry a stable human identity.** A service
   ...
   token itself: the principal is `<name>-<first 16 hex chars of sha256(jwt)>`.
   Every token refresh then produces a _different_ Horizon principal. This
   works for machine federation, where the token is long-lived and the identity
   is the service account. It does not work for a human, whose ownership, team
   membership, and audit trail must survive a token refresh.
4. **OIDC login is browser-only.** OIDC login in Horizon is a browser redirect
   flow. Its output is a session cookie, not a credential that a client can
   send as a header. A caller outside the browser flow cannot get an OIDC-based
   Horizon identity. This ADR does not consider any workaround that creates or
   reuses session cookies outside that flow.

**What would unblock contract 6.** Horizon would need to accept an OIDC access
token on the API, on a Horizon-specific header. It would need to validate the
token against the identity provider configuration that browser login already
uses. It would need to map claims to the same identity that browser login
produces.

Identifiers would then match the browser flow exactly, so ownership and audit
line up, and operators would configure nothing new. Until Horizon supports this,
no OAuth implementation here can preserve per-user Horizon RBAC, and this ADR's
decision stands.
