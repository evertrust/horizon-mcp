...
`Cookie` outright with a 400 (`src/http/credentials.ts:44-48,157-169`).
...
(`src/http/credentials.ts:324`) builds an upstream client from API-key,
...
`ServiceAccountAuthProvider.getHeaders()` (`src/auth/service-account.ts:121-126`)
