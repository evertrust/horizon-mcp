...
Most `update_*` tools are configuration CRUD tools whose update is a full-replace `PUT`, so their registrar marks them mutating-destructive. Read each tool's tier in the tables below, because a few `update_*` tools keep the mutating-safe default.

## Delete safety

Every `delete_*` and `flush_*` tool needs an `expected_name` parameter, or an object-specific `expected_<identifier>` parameter. The tool's input schema names the exact parameter. The value must match the target exactly. This echo is a safety check inside the tool, not an interactive prompt.
...
| `switch_team`                          | mutating-destructive | Replace one team with another, re-homing members and ownership |
