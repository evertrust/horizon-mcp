...
`HORIZON_URL` and the existing authentication variables are unchanged. In `mtls` mode, set `HORIZON_URL` to the internal Horizon backend. That backend must trust the forwarded certificate header, and the connection to it bypasses the Horizon nginx server.
...
bun run validate:ci     # format:check + lint + typecheck + build + docs diff + unit + LLM scenarios
```
