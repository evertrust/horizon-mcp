...
| `HORIZON_CREDENTIAL_CACHE_TTL`    | `300`       | Idle TTL in seconds for cached credentials (`30` to `3600`), measured from the last use and reset on each cache hit. Retirement blocks new reuse immediately; revalidation happens on the first request after retirement, and cleanup waits for in-flight requests. |
