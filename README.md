...
| `HORIZON_CREDENTIAL_CACHE_TTL`      | `300`       | Seconds that the server reuses a cached credential before it retires the credential and revalidates it against Horizon (`30` to `3600`). The count starts at validation, not at last use. A non-CSRF Horizon `401` or `403` that reaches the end of the authentication-retry path retires the credential at once, whatever the TTL. Before that, the Horizon client tries to re-authenticate once, unless the re-authentication backoff suppresses it. Retirement blocks new reuse at once. Revalidation happens on the first request after retirement, and cleanup waits for the in-flight requests. |
